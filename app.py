...
import pyresearch
from logic import get_meat_freshness_class, generate_meat_freshness_report, MEAT_FRESHNESS_CLASSES
from meat_freshness_agent import create_agent
import asyncio
    ...
    try:
        ...
        if len(detections) > 0:
            for i in range(len(detections)):
                ...
                # Hybrid Classification (YOLO + Color)
                class_key = get_meat_freshness_class(conf, bbox, image.shape, image)
                class_info = MEAT_FRESHNESS_CLASSES[class_key]
                
                # Shrink bounding box for display
                shrunk_bbox = shrink_box(bbox, scale=0.8)
                detections.xyxy[i] = shrunk_bbox
                
                # Override detection class for supervision annotator (using index 0,1,2,3 for our 4 classes)
                # But notice supervision needs integer IDs. We'll map them for annotation:
                class_map = {'fresh_meat': 0, 'medium_fresh': 1, 'aging_meat': 2, 'spoiled_meat': 3}
                detections.class_id[i] = class_map[class_key]
            ...
            # Use logic.py to generate rich report
            ai_report = generate_meat_freshness_report(detections_list, model_name="YOLO Hybrid")
            
            # Calculate percentages based on our hybrid results
            class_counts = {'Fresh': 0, 'Half-Fresh': 0, 'Spoiled': 0} # Compatibility with old frontend
            # Map new classes to old simple categories for the pie chart
            for d in detections_list:
                if d['class'] == 'fresh_meat': class_counts['Fresh'] += 1
                elif d['class'] == 'medium_fresh': class_counts['Half-Fresh'] += 1
                elif d['class'] == 'aging_meat': class_counts['Half-Fresh'] += 1
                elif d['class'] == 'spoiled_meat': class_counts['Spoiled'] += 1
            
            percentages = {k: (v / len(detections_list) * 100) for k, v in class_counts.items()}
            suggestion = ai_report # Send the full rich report as the suggestion
        else:
            print("No objects detected in image")
            return {'Fresh': 0, 'Half-Fresh': 0, 'Spoiled': 0}, 'No objects detected.', None, None
...
def upload_file():
    try:
        ...
        if 'file_path' in locals() and os.path.exists(file_path):
            # os.remove(file_path) # Keep file temporarily for optional agent reasoning if needed? No, let's stick to clean up
            os.remove(file_path)
...
def agent_reason():
    try:
        ...
        # Run agent analysis (async to sync for Flask simple server)
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(agent.analyze_with_reasoning(data))
        
        return jsonify(result)
    except Exception as e:
        print("Error in /reason:", traceback.format_exc())
        return jsonify({'error': str(e)}), 500

if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5004, use_reloader=False)  # Disable auto-reload